...
static void rc2d_filedialog_show(RC2D_FileDialogType type, RC2D_FileDialogCallback callback, void *userdata, const RC2D_FileDialogOptions *options) 
{
    ...
    RC2D_FileDialogCallbackData *callback_data = SDL_malloc(sizeof(RC2D_FileDialogCallbackData));
    if (!callback_data)
    {
    ...
    if (options) 
    {
        ...
        {
            sdl_filters = SDL_malloc(options->num_filters * sizeof(SDL_DialogFileFilter));
            if (!sdl_filters) 
            {
