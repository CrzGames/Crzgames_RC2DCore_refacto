 ...
 * 
 * Cette option permet d'activer le rechargement à chaud des shaders, ce qui signifie que les shaders 
 * peuvent être rechargés sans avoir à redémarrer l'application. Permet lors de la modification des source d'un shader HLSL de
 ...
 * Il est donc recommandé de la désactiver pour les versions de production et d'utiliser les shaders hors ligne (précompilés).
 * 
 ...
 * - spirv-cross-c-shared.dll : Traduit SPIR-V en MSL (Metal) + Permet les réflexions automatiques sur les ressources shaders.
 * - SDL3_shadercross.dll + SDL3_shadercross.lib OU SDL3_shadercross_static.lib : Permet la compilation en ligne de shaders.
 * - SDL3.dll : Bibliothèque runtime SDL3.
 * 
 * Pour Linux, il faudra linker/fournir les bibliothèques suivantes :
 * - libdxcompiler.so : Traduit HLSL en SPIR-V ou DXIL.
 ...
 * - libSDL3.so.0 : Bibliothèque runtime SDL3.
 * - libSDL3_shadercross.so : Permet la compilation en ligne de shaders.
 * - libvkd3d.so : Composant principal pour la conversion HLSL vers SPIR-V.
 * - libvkd3d-shader.so : Bibliothèque de traduction des shaders (HLSL -> SPIR-V).
 * - libvkd3d-utils.so : Bibliothèque utilitaire pour VKD3D.
 * 
 * Pour macOS, il faudra linker/fournir les bibliothèques suivantes :
 * - libdxcompiler.dylib : Traduit HLSL en SPIR-V ou DXIL.
 ...
 * - libSDL3.0.dylib : Bibliothèque runtime SDL3.
 * - libSDL3_shadercross.dylib : Permet la compilation en ligne de shaders.
 * - libvkd3d.dylib : Composant principal pour la conversion HLSL vers SPIR-V.
 * - libvkd3d-shader.dylib : Bibliothèque de traduction des shaders (HLSL -> SPIR-V).
